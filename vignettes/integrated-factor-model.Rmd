...
fit_dfa_pred <- function(y,
  ...
  pars <- c("x", "Z", "sigma", "log_lik", "psi", "beta", "epsilon") # removed pred
...
a = weekly_clean_data %>% select(1:7, total_sales) %>% scale
...
mod = fit_dfa_pred(y = a,
  ...
  chains = 2,
  thin = 1,
  control = list(adapt_delta = 0.9, max_treedepth = 5),
  nu_fixed = 20,
  est_correlation = FALSE,
  estimate_nu = T,
  estimate_trend_ar = F,
...
stan_trace(mod, "beta[6]")
post$x[, , ] %>% apply(c(2,3), mean) %>% t %>% .[, 2] %>% ts.plot()

```
```{r}
mcmc_parcoord(mod, np = np, pars = vars(starts_with("beta")))
```
