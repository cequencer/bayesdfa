...
```{r}
stan_trace(fuck, "rates")
```

And it converged!!!!!

```{r}
post = rstan::extract(fuck)
```

```{r}
library(loo)
log_lik <- extract_log_lik(fuck, merge_chains = FALSE)
r_eff <- relative_eff(exp(log_lik))
leeloo <- loo(log_lik, r_eff = r_eff, cores = 2)
leeloo
```
...
fuckfuck = rstan::extract(fucktwelve)
