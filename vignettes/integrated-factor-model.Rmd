...
stan_data = c(factor_data, channel_data, n_holdout = 0, residualize = 0)

```

And finally, to start sampling -- with no residualization:
...
post_nores = rstan::extract(fit_nores)
...
post_res = rstan::extract(fit_res)
```


```{r}
library(loo)
log_lik_res <- extract_log_lik(fit_res, merge_chains = FALSE)
r_eff_res <- relative_eff(exp(log_lik_res))
leeloo_res <- loo(log_lik_res, r_eff = r_eff_res, cores = 2)

log_lik_nores <- extract_log_lik(fit_nores, merge_chains = FALSE)
r_eff_nores <- relative_eff(exp(log_lik_nores))
leeloo_nores <- loo(log_lik_nores, r_eff = r_eff_nores, cores = 2)

loo_compare(leeloo_res, leeloo_nores)

loo::loo_model_weights(list(leeloo_res, leeloo_nores))
```

```{r}
ts.plot(weekly_clean_data$total_sales)
abline(v = loo::pareto_k_ids(leeloo_nores, threshold = 1), col = "darkred")
```
