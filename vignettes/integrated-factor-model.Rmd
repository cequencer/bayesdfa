...
### testing consistency of the rates

```{r}
chan_names$consistency = colnames(current_data)[3:24]

factor_data$consistency = generate_factor_stan_data(depvar_name, 
                                              chan_names$consistency, 
                                              num_trends = 3, estimate_nu = T, 
                                              zscore = T)


channel_data$consistency = generate_obs_stan_data(depvar_name, 
                                                  chan_names$consistency, 
                                                  num_trends = 3, 
                                                  config_dir, 
                                                  upscale_trends = T)

stan_data$consistency = c(factor_data$consistency, channel_data$consistency, 
                               n_holdout = 0, residualize = 0, upscale_trends = 0)
...
```{r}
fit$consistency$one = vb(
  stanmodels$dfa_pred,
  stan_data$consistency,
  seed = 1,
  pars = c("channel_spend_impact", "log_lik", "x", "y_pred", "Z", "rates", "Px_diag")
)

fit$consistency$two = vb(
  stanmodels$dfa_pred,
  stan_data$consistency,
  seed = 1,
  pars = c("channel_spend_impact", "log_lik", "x", "y_pred", "Z", "rates", "Px_diag")
...
post$consistency$one = rstan::extract(fit$consistency$one)
post$consistency$two = rstan::extract(fit$consistency$two)

fit$consistency$three = vb(
  stanmodels$dfa_pred,
  stan_data$consistency,
...
post$consistency$three = rstan::extract(fit$consistency$three)
post$consistency$four = rstan::extract(fit$consistency$four)
```




### testing to see if the rates are the same

```{r}
cbind(
  post$consistency$one$rates %>% colMeans,
  post$consistency$two$rates %>% colMeans
) 
...
```{r}
post$consistency$one$rates[, 6] %>% hist(col = "lightgrey")
post$consistency$two$rates[, 6] %>% hist(col = scales::alpha("darkred", 0.5), add = T)
```

Is there any relationship to spend levels and whether or not the rates are consistent?

```{r}
rate_inconsistencies = cbind.data.frame(
  first = post$consistency$one$rates %>% colMeans,
  second = post$consistency$two$rates %>% colMeans
) %>% 
  mutate(delta = abs(first-second)) %>% 
  mutate(idx = row_number()-num_factors) %>% 
  filter(idx > 0)

spend_amounts = cbind.data.frame(
  spend_amount = stan_data$consistency$predictors %>% apply(2, mean)
) %>% 
  mutate(idx = 1:n())

tgthr = left_join(
  rate_inconsistencies, 
  spend_amounts, by = "idx"
) 

tgthr %>% head

summary(lm(scale(delta) ~ scale(spend_amount), tgthr))
plot(scale(delta) ~ scale(spend_amount), tgthr)
...
```{r}
plot_comp_between_channels(15, post$consistency, 3)
```


Well that sucks ... checking loo

```{r}
log_lik$consistency$one <- extract_log_lik(fit$consistency$one, merge_chains = FALSE)
r_eff$consistency$one   <- relative_eff(exp(log_lik$consistency$one))
leeloo$consistency$one  <- loo(log_lik$consistency$one, r_eff = r_eff$consistency$one, cores = 2)

log_lik$consistency$two <- extract_log_lik(fit$consistency$two, merge_chains = FALSE)
r_eff$consistency$two   <- relative_eff(exp(log_lik$consistency$two))
leeloo$consistency$two  <- loo(log_lik$consistency$two, r_eff = r_eff$consistency$two, cores = 2)

loo_compare(leeloo$consistency)
loo::loo_model_weights(leeloo$consistency)
leeloo$consistency$two
```
...
```{r}
sim_steps = 200
sim_dat_raw = sim_dfa(num_trends = 3, num_years = sim_steps, num_ts = 11)
...
stan_data$simplus = stan_data$sim

set.seed(1)
effect = cumsum(rnorm(sim_steps, 0, sd = 0.5))

stan_data$simplus$depvar = stan_data$sim$depvar + effect * sim_dat[, 2]

ts.plot(cbind(stan_data$simplus$depvar, stan_data$sim$depvar), 
        col = c("darkred", "darkblue"), lty = 1)
legend("topleft", c("with effect", "original"), col = c("darkred", "darkblue", "darkred"), lty = 1)
...
post$simplus = rstan::extract(fit$simplus)
```
...
par(mfrow=c(1,2))
m = post$simplus$channel_spend_impact %>% apply(c(2,3), mean) %>% .[, actual_idx] 
s = post$simplus$Px_diag %>% apply(c(2,3), mean) %>% sqrt %>% .[, actual_idx]

ts.plot(cbind(m+2*s, m, m-2*s, effect),
        col = c("darkred", "black", "darkred", "darkgreen"), lty = c(2,1,2, 1),
        main = "Channel With Effect")
legend("topleft", c("estimated", "actual"), col = c("black", "darkgreen"), lty = 1)


m = post$simplus$channel_spend_impact %>% apply(c(2,3), mean) %>% .[, no_effect_idx] 
s = post$simplus$Px_diag %>% apply(c(2,3), mean) %>% sqrt %>% .[, no_effect_idx]

ts.plot(cbind(m+2*s, m, m-2*s, effect),
        col = c("darkred", "black", "darkred", "darkgreen"), lty = c(2,1,2, 1),
        main = "Channel With No Effect")
legend("topleft", c("estimated", "actual"), col = c("black", "darkgreen"), lty = 1)
...
0. need to update rstan > 19.2 and stanheaders > 19.2
1. need to write the functions that compile the data to account for the new data the stan model requires
2. need to overwrite the stan model 
3. need to change the `sampling` call to a `vb` call


### hard

1. need to change the model s.t. it can accept a holdout period

$$ spend = Z * factors$$
if you want to infer what the factors must have been if you observe spend

$$ factors = Z^{-1} * spend$$

2. either
  - need to contrain the rates s.t. they are consistent from run to run
  - need to write an update model
